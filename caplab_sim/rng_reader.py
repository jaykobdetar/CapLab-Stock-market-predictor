...
1.  The RNG seed at ``Misc+0x7C`` (same location handled by the top-level
    ``rng.py`` reader; the Misc pointer is discovered via the call-site
    scan in :func:`_find_misc_pointer`).
...
# Global RNG call counter (not used by this reader; kept for parity with
# the top-level ``rng.py`` and for future cross-checks).
...
def _find_misc_pointer(pid: int, base: int) -> Optional[int]:
    ...
    (the last instruction before the CALL is the one that matters) and
    the heuristic used by the top-level ``rng.py`` reader.
    ...
    seed so we can't test it structurally; if this heuristic still
    mis-fires on some build, compare ``read_rng_seed()`` against the
    top-level ``rng.py`` output and widen the scan.
    """
